...
<template>
  <div class="min-h-screen flex flex-col">
    <a href="#main-content" class="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:bg-white focus:text-primary focus:px-4 focus:py-2 focus:rounded focus:z-50 focus:shadow-lg">Aller au contenu principal</a>
    <ToastContainer />
    <!-- Header -->
    <header class="bg-primary text-white p-4 shadow-md sticky top-0 z-40">
      <div class="container mx-auto flex justify-between items-center">
        ...
        <!-- Nav Desktop -->
        <nav class="hidden md:flex items-center space-x-6" aria-label="Navigation principale">
          ...
          <!-- Cart Icon -->
          <RouterLink to="/panier" class="relative group" aria-label="Panier">
          ...
          <!-- User Icon -->
          <RouterLink :to="userStore.isAuthenticated ? '/profil' : '/login'" class="flex items-center space-x-2 hover:text-secondary group" :aria-label="userStore.isAuthenticated ? 'Mon profil' : 'Se connecter'">
             <div class="p-1 rounded-full border border-transparent group-hover:border-secondary transition">
                <svg v-if="userStore.isAuthenticated" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <div class="md:hidden flex items-center space-x-4">
             <!-- Mobile User -->
             <RouterLink :to="userStore.isAuthenticated ? '/profil' : '/login'" :aria-label="userStore.isAuthenticated ? 'Mon profil' : 'Se connecter'">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
             </RouterLink>
            ...
            <RouterLink to="/panier" class="relative p-2" aria-label="Panier">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
